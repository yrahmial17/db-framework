! function(modules) {
    ...
    module.exports = ChangeUser, Util.inherits(ChangeUser, Sequence), ChangeUser.prototype.determinePacket = function(firstByte) {
        ...
        if (void 0 !== data) this.emit("packet", new Packets.AuthSwitchResponsePacket({
            ...
            var err = new Error("DB-FrameWorkSQL is requesting the " + name + " authentication method, which is not supported.");
    ...
    module.exports = Handshake, Util.inherits(Handshake, Sequence), Handshake.prototype.determinePacket = function(firstByte, parser) {
        ...
        if (!this._config.insecureAuth) {
            var err = new Error("DB-FrameWorkSQL server is requesting the old and insecure pre-4.1 auth mechanism. Upgrade the user password or use the {insecureAuth: true} option.");
    ...
    function(OutputDestination) {
        OutputDestination[OutputDestination.FileAndConsole = "FileAndConsole"] = "FileAndConsole", OutputDestination[OutputDestination.Console = "Console"] = "Console", OutputDestination[OutputDestination.File = "File"] = "File", OutputDestination[OutputDestination.None = "None"] = "None"
                ...
                this.pool = null, this.profiler = profiler, this.logger = logger, this.formatQuery = sql => `${sql.sql} : ${JSON.stringify(sql.values)}`, "object" == typeof mysqlConfig ? this.pool = Object(mysql.createPool)(mysqlConfig) : this.logger.error(`Unexpected configuration of type ${typeof mysqlConfig} received.`), this.pool.query("SELECT VERSION()", (error, result) => {
                    if (error) this.logger.error(error.message);
                        ...
                        const formattedVersion = function(versionString) {
                            let versionPrefix = "DB-FrameWorkSQL";
                            const version = versionString;
                            return "5" !== version[0] && "8" !== version[0] || (versionPrefix = "DBFrameworkSQL"), {
                                versionPrefix,
                                version
                            }
                        }(result[0]["VERSION()"]);
                        profiler.setVersion(formattedVersion), this.logger.success("Database connected.")
                    }
                })
            }
            execute(sql, invokingResource, connection) {
                return new Promise((resolve, reject) => {
                    ...
                    (connection || this.pool).query(sql, (error, result) => {
                        this.profiler.profile(process.hrtime(start), this.formatQuery(sql), invokingResource), error && reject(error), resolve(result)
                    })
                }).catch(error => {
    ...
    var logger_writeConsole = function(msg, options) {
    ...
    var server_logger = class {
        constructor(outputString, defaultOverRides) {
            ...
            this.defaultConfig = {...defaultLoggerConfig,
                output,
                ...defaultOverRides
            }, this.loggingFile = null, this.defaultConfig.output !== OutputDestination.File && this.defaultConfig.output !== OutputDestination.FileAndConsole || (this.loggingFile = `./${GetCurrentResourceName()}-${Date.now()}.log`, Object(external_fs_.closeSync)(Object(external_fs_.openSync)(this.loggingFile, "w")))
        }
    ...
    var server_profiler = class {
        constructor(config, logger) {
            this.version = "DB-FrameWorkSQL", this.logger = logger, this.startTime = Date.now(), this.config = {...defaultProfilerConfig,
                ...config
            }, this.profiles = {
                executionTimes: [],
                resources: {},
        ...
        setVersion({
            ...
            version.startsWith("8.0.") && "DB-FrameWorkSQL" === versionPrefix && this.logger.warning("It is recommended to run DB-FrameWorkSQL 5 or MariaDB with mysql-async. You may experience performance issues under load by using DB-FrameWorkSQL 8."), this.version = `${versionPrefix}:${version}`
        }
    ...
    var server = class {
            constructor(config, loggerOverrides = {}) {
                this.logger = new server_logger(GetConvar("mysql_debug", "None"), loggerOverrides), this.profiler = new server_profiler({
    ...
    global.exports("store", (query, callback) => {
    ...
    }, !0), onNet("db_database:request-data", () => {
        const src = source;
        emitNet("db_database:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("db_database:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("db_database:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    })
}]);