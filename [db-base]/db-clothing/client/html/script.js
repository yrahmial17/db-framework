...
whitelisted["male"] = {
    jackets:[378,379,380,381,382,383,384,385,386,387,388,396,397,398,413,414,415,416,417,418],
    undershirts:[194,195,196,197,198,199,200,201,202,203,205],
    legs:[149,150],
    decals:[85,86,87,88,89,90,91,92,97,98],
    vest:[54,55,66,67,68,69,70,71,72,73,74,75,76,77],
    hats:[20,21,22,23,24,25,28,56],
}
whitelisted["female"] = {
    jackets:[397,398,399,400,401,402,403,404,405,406,407,408,423,438,439,440,441,442,433,],
    undershirts:[232,233,234,235,236,237,238,239,244,245,247,],
    legs:[156,157,158],
    decals:[95,96,97,98,99,100,105,106],
...
$(function () {
    ...
    function CloseMenu(save, paymentType) {
        $.post('https://db-clothing/escape', JSON.stringify({
          save: save,
          paymentType,
    ...
    $(document).on('contextmenu', function() {
        $.post('https://db-clothing/togglecursor', JSON.stringify({}));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('https://db-clothing/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothing_shop')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('https://db-clothing/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('https://db-clothing/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('https://db-clothing/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('https://db-clothing/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('https://db-clothing/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('https://db-clothing/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('https://db-clothing/switchcam', JSON.stringify({name: 'cam'}))
    })

    $('.backup_save').on('click', function() {
        $.post('https://db-clothing/getPlayerFace')
    })
    ...
    $('.backup_load').on('click', function() {
        $.post('https://db-clothing/setPlayerFace',$("#backupJsonInput").val())
    })

    $('.tog_hat').on('click', function() {
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('https://db-clothing/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('https://db-clothing/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('https://db-clothing/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('https://db-clothing/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('https://db-clothing/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('https://db-clothing/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('https://db-clothing/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('https://db-clothing/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
